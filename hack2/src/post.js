...
function Post(props) {
  ...
  const getPostDetail = async (pid) => {
    const data = await instance.get('/postDetail', {params: {pid: pid}})
    console.log('fe postDetail data', data.data);
  ...
  const delPost = async (pid) => {
    const data = await instance.delete('/post', {params: {pid: pid}})
    console.log('fe delPost', data.data.message);
